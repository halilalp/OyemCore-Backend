...
namespace OyemCore.BusinessLayer.Services
{
    public class IzinService : IIzinService
    {
        ...
        public bool ApproveIzinRequest(int kullaniciID, int izinOnayID)
        {
            ...
            if (request == null) throw new InvalidOperationException("Izin talebi bulunamadi.");
            ...
            if (request.SurecDurum == "IKONAY")
            {
                ...
                if (approvalRecord != null)
                {
                    ...
                    approvalRecord.Aciklama = "IK Tarafindan Onaylandi";
                }

                // Final Approval Details
                request.Durum = true;
                request.SurecDurum = "ONAYLANDI";
                request.SonDurumBilgi = $"Talep {user.AdSoyad} ile tamamlandi. ({DateTime.Now:dd.MM.yyyy HH:mm})";
            ...
            else
            {
                ...
                if (amirInfo != null)
                {
                    ...
                    {
                        nextAmir = amir2;
                        stateInfo = "2. Amir Onayi Bekliyor";
                    }
                    else if (user.SicilNo == amir2 && !string.IsNullOrEmpty(amir3))
                    {
                        nextAmir = amir3;
                        stateInfo = "3. Amir Onayi Bekliyor";
                    }
                }
                ...
                if (approvalRecord != null)
                {
                    approvalRecord.Durum = true;
                    approvalRecord.IslemTar = DateTime.Now;
                    approvalRecord.Aciklama = "Onaylandi";
                }

                if (string.IsNullOrEmpty(nextAmir))
                {
                    // Manager hierarchy completed! Send to HR (IK)
                    request.BekleyenOnay = null;
                    request.SurecDurum = "IKONAY";
                    request.SonDurumBilgi = $"IK Onay Kutusunda ({DateTime.Now:dd.MM.yyyy HH:mm})";
