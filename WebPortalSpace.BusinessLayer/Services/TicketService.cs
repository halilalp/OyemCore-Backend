...
namespace WebPortalSpace.BusinessLayer.Services
{
    public class TicketService : ITicketService
    {
        ...
        public (bool Success, object Data, string Message) InitConfig(int kullaniciID)
        {
            ...
            if (usr == null) return (false, null, "Kullanıcı bulunamadı.");
            ...
            foreach (var status in statuses)
            {
                var tickets = filteredQueryList
                    ...
                    .Select(o => new
                    {
                        ...
                        o.SorumluSicilNo,
                        SorumluAd = o.SorumluPersonelAd ?? "Atanmamış",
        ...
        public string SaveTicket(int kullaniciID, tb_Ticket ticket)
        {
            ...
            if (usr == null) throw new InvalidOperationException("Kullanıcı bulunamadı.");
            ...
            if (ticket.ID == 0)
            {
                ...
                BelgeTarihceKaydet(ticket.TakipKodu, "Ticket Oluşturuldu", $"Yeni kayıt açıldı. (Yapan: {usr.AdSoyad})");
                _ = _pushNotificationService.NotifyNewTicketAsync(ticket.ID);
            }
            else
            {
                var existing = _context.tb_Ticket.FirstOrDefault(t => t.ID == ticket.ID);
                if (existing == null) throw new InvalidOperationException("Güncellenecek ticket bulunamadı.");
        ...
        public bool UpdateTicketStatus(int kullaniciID, int ticketID, string yeniDurum, int? draggedID)
        {
            ...
            if (t != null && t.SurecDurumu != yeniDurum)
            {
                ...
                BelgeTarihceKaydet(t.TakipKodu, "Süreç Değişikliği", $"'{yeniDurum}' aşamasına geçildi. (Yapan: {usr.AdSoyad})");
                _ = _pushNotificationService.NotifyTicketStatusChangedAsync(t.ID, "", yeniDurum);
        ...
        public bool AssignTicket(int kullaniciID, int ticketID, string sicilNo)
        {
            ...
            if (t != null)
            {
                ...
                BelgeTarihceKaydet(t.TakipKodu, "Atama İşlemi", $"Sorumlu: {adSoyad} (Atayan: {usr.AdSoyad})");
        ...
        public object GetTicketDetail(int ticketID)
        {
            ...
            var sorumluAd = _context.tb_Personel
                .AsNoTracking()
                .FirstOrDefault(p => p.SicilNo == t.SorumluSicilNo)?.AdSoyad ?? "Atanmamış";
        ...
        public bool SaveComment(int kullaniciID, int ticketID, string aciklama)
        {
            ...
            if (t != null)
            {
                ...
                BelgeTarihceKaydet(t.TakipKodu, "Yeni Yorum", $"Ticketa yeni gelişme eklendi. (Yapan: {usr.AdSoyad})");
                ...
                try
                {
                    ...
                    if (draggedID.HasValue && !string.IsNullOrEmpty(yeniDurum))
                    {
                        ...
                        if (dt != null && dt.SurecDurumu != yeniDurum)
                        {
                            ...
                            _context.SaveChanges();
                            BelgeTarihceKaydet(dt.TakipKodu, "Süreç Değişikliği", $"'{yeniDurum}' aşamasına taşındı. (Yapan: {usr.AdSoyad})");
                        }
                    }
        ...
        public string DeleteTicket(int kullaniciID, int id, string webRootPath)
        {
            ...
            if (t == null) return "Kayıt bulunamadı.";
        ...
        public object GetDashboardStats(int kullaniciID, string sirketKodu, int ay, int fltYil, int fltAy)
        {
            ...
            if (staleCount > 0)
                insights.Add(new { type = "danger", icon = "ki-time", text = $"{staleCount} bilet 2 haftadan uzun süredir sonuçlanmamış — Takip gerekebilir." });
