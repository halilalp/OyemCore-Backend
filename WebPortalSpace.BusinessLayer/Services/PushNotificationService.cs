...
namespace WebPortalSpace.BusinessLayer.Services
{
    public class PushNotificationService : IPushNotificationService
    {
            ...
            try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    await SendToUserBySicilNoAsync(
                        leave.BekleyenOnay,
                        "Yeni İzin Talebi",
                        $"{requesterName} yeni bir izin talebi oluşturdu. Onayınız bekleniyor.",
            ...
            try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    foreach (var hrUser in hrUsers)
                    {
                        ...
                        await SendToUserBySicilNoAsync(
                            hrUser.SicilNo,
                            "İK Onayı Bekleyen İzin",
                            $"Yeni bir izin talebi ({leave.BelgeNo}) İK onayınızı bekliyor.",
                            new { type = "izin", screen = "IzinScreen", code = leave.BelgeNo }
                        );
                    }
                }
            ...
            try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    await SendToUserBySicilNoAsync(
                        ...
                        $"İzin talebiniz ({leave.BelgeNo}) {actionUserName} tarafından reddedildi.",
                        new { type = "izin", screen = "IzinScreen", code = leave.BelgeNo }
                    );
                }
            }
        ...
        public async Task NotifyLeaveRequestCompletedAsync(int leaveRequestId, int actionUserId)
        {
            try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    var actionUser = await context.tb_Kullanici.AsNoTracking().FirstOrDefaultAsync(u => u.KullaniciID == actionUserId);
                    var actionUserName = actionUser?.AdSoyad ?? "İK Yetkilisi";

                    await SendToUserBySicilNoAsync(
                        leave.KayitSicil,
                        "İzin Talebiniz Tamamlandı",
                        $"İzin talebiniz ({leave.BelgeNo}) {actionUserName} tarafından onaylandı.",
                        new { type = "izin", screen = "IzinScreen", code = leave.BelgeNo }
                    );
                }
            }
        ...
        public async Task NotifyNewTalepAsync(int talepId)
        {
            try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    string typeLabel = tur == "BAKIM" ? "Bakım" : tur;
        ...
        public async Task NotifyTalepSorumluAtandiAsync(int talepId)
        {
            try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    string typeLabel = talep.TalepTurKodu == "BAKIM" ? "Bakım" : talep.TalepTurKodu;
        ...
        public async Task NotifyTalepGelismeAsync(int talepId, int actionUserId, string description)
        {
            try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    string typeLabel = talep.TalepTurKodu == "BAKIM" ? "Bakım" : talep.TalepTurKodu;
                    string title = $"{typeLabel} Talebi Gelişmesi";
                    string body = $"'{talep.Konu}' konulu talebe ({talep.TalepKodu}) yeni bir gelişme eklendi.";
        ...
        public async Task NotifyTalepClosedAsync(int talepId)
        {
            try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    string typeLabel = talep.TalepTurKodu == "BAKIM" ? "Bakım" : talep.TalepTurKodu;
                    string title = $"{typeLabel} Talebiniz Tamamlandı";
                    string body = talep.TalepTurKodu == "BAKIM"
                        ...
                        : $"'{talep.Konu}' konulu talebiniz ({talep.TalepKodu}) tamamlandı.";
        ...
        public async Task NotifyTicketStatusChangedAsync(int ticketId, string oldStatus, string newStatus)
        {
            try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    string body = $"'{ticket.Baslik}' başlıklı ticketınızın ({ticket.TakipKodu}) durumu '{newStatus}' olarak güncellendi.";
        ...
        public async Task NotifyTicketGelismeAsync(int ticketId, int actionUserId, string comment)
        {
            try
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    string title = "Destek Talebi Gelişmesi (Ticket)";
                    string body = $"'{ticket.Baslik}' başlıklı destek talebine ({ticket.TakipKodu}) yeni bir yorum/gelişme eklendi.";
